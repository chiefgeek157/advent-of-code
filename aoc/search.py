...
def djikstra(start: Node, f_neighbors: NeighborFunc) -> SearchResult:
    """Return the min dist and min path from start node.

    The score function, f, for a given node, x, is given by:

        f(x) = f(x-1) + d

    where f(x) is the score of node x, f(x-1) is the cumulatrive score for the
    path leading from start to x, and d is the distance from x-1 to x.
    ...
    #
    # dists is a cache of dists by node
    #
    ...
    min_cum_dist = math.inf
    final = None
    while priority_queue:

        print(f'Prioriy queue length {len(priority_queue)}')
        ...
        cum_dist, depth, node = hq.heappop(priority_queue)
        if node is not None:
            queued_entries.pop(node)
            print(f'Visiting node {node} with dist {dists[node]}')

            if cum_dist < min_cum_dist:
                # This is the best candidate found so far
                print(f'  - New shortest distance {cum_dist} at node {node}')
            ...
            for next_node, dist in neighbors:
                ...
                next_cum_dist = cum_dist + dist
                next_depth = depth + 1

                # Initialize the cached score to infinity if needed
                if next_node not in dists:
                    dists[next_node] = math.inf
                if next_cum_dist < dists[next_node]:

                    # This is a better (or perhaps first) score for next_node
                    print(f'  - Setting distance of node {next_node} to '
                        f'{next_cum_dist} and predecessor {node}')
                    dists[next_node] = next_cum_dist

                    # Set predecessor to node
                    predecessors[next_node] = node

                    next_entry = [next_cum_dist, next_depth, next_node]
                    add_entry = False
                    if next_node not in queued_entries:
                        ...
                        if next_cum_dist < queued_entry[1]:
                            # This is a better cum dist for this node
                            # Mark the queued entry as removed and add
                            # the new item
                            print(f'  - Replacing entry for node {next_node} '
                                f'from {queued_entry[1]} to {next_cum_dist}')
                            queued_entry[2] = None
                            add_entry = True
                    if add_entry:
                        print(f'  - Adding node {next_node} to queue with '
                            f'cum dist {next_cum_dist} and depth {next_depth}')
